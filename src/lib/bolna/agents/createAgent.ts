...
export async function createAgent(agentData: AnyObj) {
  try {
    ...
    // ⭐️ CRITICAL FIX: The voice object is now expected to be nested under voiceObject
    ...
    // --- Dynamic LLM Agent Config (omitted as it was not the focus of the fix) ---
    ...
    if (vectorId) {
      llm_agent_config = {
        ...
        llm_config: { provider: "openai", family: "openai", model: agentData.llm_model || "gpt-3.5-turbo", temperature: agentData.temperature || 0.1, },
        vector_store: { provider: "lancedb", provider_config: { vector_id: vectorId } }
      };
    } else {
      llm_agent_config = {
    ...
    // --- Main Agent Config (omitted as it was not the focus of the fix) ---
      ...
      tasks: [
        {
          ...
          tools_config: {
            ...
            transcriber: { provider: "deepgram", model: "nova-2", language: agentData.language || "en", stream: true, sampling_rate: 16000, encoding: "linear16", endpointing: 100, },
          ...
          task_config: { hangup_after_silence: 10, incremental_delay: 400, number_of_words_for_interruption: 2, call_terminate: agentData.max_duration_seconds || 90, },
        },
      ],
    };

    // --- 2. Build the Bolna agent_prompts (omitted) ---
    const agent_prompts: BolnaAgentPrompts = { task_1: { system_prompt: agentData.systemPrompt, }, };
    
    // --- 3. Build the final request body (omitted) ---
    const requestBody = { agent_config, agent_prompts, };

    // --- 4. Call the Bolna API (omitted) ---
    ...
    const agent = new Agent({
      ...
      // ⭐️ CRITICAL FIX: Extract required fields from voiceObject for DB save
      agentId: agentId,
      voiceProvider: agentData.voiceObject.provider, // NEW
      voiceModel: agentData.voiceObject.model,     // NEW
      voiceId: agentData.voiceObject.id,      // The Bolna UUID
      voiceName: agentData.voiceObject.name,  // The voice's name (e.g., "Matthew")
      language: agentData.language,         

      knowledgeVectorIds: agentData.knowledgeVectorIds || [],
      systemTools: getDefaultSystemTools(),
      voiceObject: undefined, // Do not save the full object to the DB
    });
    await agent.save();
