// src/app/dashboard/new-agent/page.tsx
"use client";
...
import {
    ...
    PlayCircle, PauseCircle, Server
} from "lucide-react";
import { Label } from "@radix-ui/react-label";


// --- Schema reverted: voice_id is required again ---
const agentSchema = z.object({
    name: z.string().min(3, "Name must be at least 3 characters"),
    description: z.string().optional(),
    voice_id: z.string().min(1, "Please select a voice"), // <-- This is required
...
// ... (agentTemplates, availableTools, llmModels, languages constants remain the same) ...
...
// Define the Voice type to be safer
type Voice = {
    id: string;
    voice_id: string;
    provider: string;
    ...
    model: string;
    accent: string;
    tags?: string; // tags is used for filtering but doesn't exist in the schema, using 'accent' instead
    demo?: string; // assuming a demo URL exists for playback
};

export default function NewAgent() {
    const router = useRouter();
    const { user } = useAuth();
    // This hook now correctly fetches voices
    const { data, error, isLoading } = useSWR<{ voices: Voice[] }>("/api/voices", fetcher);

    const [creatingAgent, setCreatingAgent] = useState(false);
    const [voiceSearch, setVoiceSearch] = useState("");
    // NEW STATE: Filter voices by provider
    const [selectedProvider, setSelectedProvider] = useState<string>("all"); // 'all' or a specific provider name
    
    // --- START: Re-enabled Voice Playback Logic ---
    ...
    // --- END: Re-enabled Voice Playback Logic ---
    ...
    // Filtered voices logic combined with search
    const filteredVoices = allVoices.filter(v => {
        // Provider Filter
        const providerMatch = selectedProvider === "all" || v.provider === selectedProvider;

        // Search Filter (on name, accent, and provider)
        const name = v.name || "";
        const accent = v.accent || ""; // Use 'accent' as it's in the response schema
        const provider = v.provider || "";
        const search = voiceSearch.toLowerCase();
        
        const searchMatch = name.toLowerCase().includes(search) ||
            accent.toLowerCase().includes(search) ||
            provider.toLowerCase().includes(search);

        return providerMatch && searchMatch;
    });

    // --- Re-enabled playing logic ---
    const handlePlayVoice = (e: React.MouseEvent, voiceId: string, demoUrl?: string) => {
        e.stopPropagation(); // Stop click from selecting the voice
        if (!audioRef.current || !demoUrl) {
            console.warn("No demo URL for this voice.");
            return;
        }
        ...
        if (playingVoiceId === voiceId) {
            ...
            // Stop current playback if a different voice is playing
    ...
    // Function to handle provider change and reset voice selection
    const handleProviderChange = (providerId: string) => {
        setSelectedProvider(providerId);
        // Clear selected voice ID when changing provider to prevent submitting an incompatible voice
    ...
    // --- onSubmit reverted to finding the selectedVoice ---
    const onSubmit = async (payload: z.infer<typeof agentSchema>) => {
        try {
          ...
          if (!selectedVoice) {
              console.error("No voice selected or voice not found in allVoices array");
          ...
          const agentData = {
            ...
            voiceObject: selectedVoice, // <-- SEND THE ENTIRE OBJECT
    ...
    // ... (rest of the component logic for templatesByCategory and animations) ...
                ...
                <div className="container mx-auto px-4 sm:px-6 py-8">
                    <div className="max-w-6xl mx-auto">
                        ...
                        <motion.div initial="hidden" animate="visible" variants={containerVariant}>
                            {/* ... (Template selection code remains the same) ... */}
                            ...
                            <Form {...form}>
                                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-12">
                                    <motion.div variants={fadeInUpVariant}>
                                        ...
                                        <Tabs defaultValue="basic" className="w-full">
                                            ...
                                            <div className="mt-6">
                                                        ...
                                                        <CardContent className="space-y-6">
                                                            ...
                                                            <FormField control={form.control} name="voice_id" render={({ field }) => (
                                                                <FormItem>
                                                                    ...
                                                                    {/* --- END: NEW PROVIDER DROPDOWN --- */}
                                                                    
                                                                    <FormControl>
                                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[300px] overflow-y-auto p-1">
                                                                            {isLoading ? <p className="text-[#A7A7A7]">Loading voices...</p> : 
                                                                             error ? <p className="text-red-400">Failed to load voices.</p> :
                                                ...
                                                {/* ... (Rest of the TabsContent for behavior, knowledge, advanced remains the same) ... */}
                                    ...
                                    {/* ... (Form submission buttons remain the same) ... */}
                                    <motion.div variants={fadeInUpVariant} className="flex justify-end space-x-4">
                                        ...
                                        <Button type="submit" disabled={creatingAgent || !form.formState.isValid || !form.getValues("voice_id")} className="gap-2 min-w-[160px] bg-[#A7B3AC] text-[#111111] hover:bg-[#A7B3AC]/90 font-bold">
